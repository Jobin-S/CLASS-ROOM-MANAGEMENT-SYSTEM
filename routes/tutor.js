...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    tutorHelpers
      .updateProfilePic(req.session.tutor.email, fileName)
      .then(() => {
        cb(null, fileName);
      });
  },
...
router.get("/profile", verifyLogin, async (req, res) => {
  let tutorProfile = await tutorHelpers.getTutorProfile(
...
router.get("/login", (req, res) => {
  if (req.session.tutorLoggedIn) {
    if (req.session.previousPath) {
      ...
      location = req.session.tutorRedirectTo
        ? "/tutor" + req.session.tutorRedirectTo
        : "/tutor/dashboard";
    }
...
router.post("/login", (req, res) => {
  tutorHelpers.doLogin(req.body).then((response) => {
    if (response.status) {
      ...
      if (req.session.previousPath) {
        location =
          "/tutor" + req.session.previousPath === ""
            ? "/Dashboard"
...
router.get("/all-students/:id", verifyLogin, async (req, res) => {
  let tutorName = await tutorHelpers.getTutorName(req.session.tutor.email);
  let image = await tutorHelpers.getProfilePic(req.session.tutor.email);
  let assignments = await tutorHelpers.getSingleStudentAssignment(
    req.params.id
  );
...
router.get("/assignment/:id", verifyLogin, async (req, res) => {
  ...
  let assignment = await tutorHelpers.getSubmittedAssignment(
    req.params.id,
    req.session.currentStudentId
  );
...
router.post(
  ...
  (req, res) => {
    ...
    tutorHelpers
      .addNote(req.body, req.session.NoteFilePdf, req.session.NoteFileVideo)
      .then(() => {
        res.end();
      });
  }
...
router.post("/gallery", uploadGallery.single("img"), (req, res) => {
  ...
  tutorHelpers.addGallery(req.file.filename, req.body.title).then(res.end());
});
